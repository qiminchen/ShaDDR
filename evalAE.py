...
def eval_IOU(config):
    ...
    # change these three
    result_dir = "/mnt/sda/DECOR-GAN-512/03001627/geometry_quantitative"
    content_result_dir = result_dir + "/single_res_content_output_for_geo_eval"
    style_result_dir = result_dir + "/style_output_for_geo_eval"
    if not os.path.exists(result_dir):
        print("ERROR: result_dir does not exist! " + result_dir)
        exit(-1)

    # change this
    output_dir = result_dir + "/single_res_eval_output"
    ...
    for content_id in range(dataset_len):
        voxel_model_file = open(content_result_dir + "/content_" + str(content_id) + "_coarse.binvox", 'rb')
        ...
        for style_id in range(styleset_len):
            voxel_model_file = open(content_result_dir + "/output_content_" + str(content_id) + "_style_" + str(style_id) + ".binvox", 'rb')
            ...
            output_shape_coarse_tensor = F.max_pool3d(output_shape_tensor, kernel_size=8, stride=8, padding=0)
        ...
        for patch_id in range(1, patch_num):
            ...
            for compare_id in range(0, style_id + 1):
                ...
                if compare_id != style_id:
                    notduplicated = torch.bitwise_xor(this_patches_tensor[patch_id:patch_id + 1],
                                                      compare_patches_tensor[:dict_style_patches_len[compare_id]]).any(1).all().item()
                else:
...
def eval_LP_Div_IOU(config):
    ...
    #  change this
    patches_dir = result_dir + "/unique_patches"
    if not os.path.exists(patches_dir):
        ...
        for style_id in range(styleset_len):
            ...
            for patch_id in range(patch_num):
                for compare_id in range(styleset_len_original):
                    ...
                    ious = torch.sum(torch.bitwise_and(patch_tensor, patches_tensor), dim=(1, 2, 3), dtype=torch.int).float() / torch.sum(
                        torch.bitwise_or(patch_tensor, patches_tensor), dim=(1, 2, 3), dtype=torch.int).float()
...
def eval_LP_Div_Fscore(config):
    ...
    for content_id in range(dataset_len):
        for style_id in range(styleset_len):
            ...
            patches_edge = np.zeros([buffer_size, patch_size, patch_size, patch_size], np.uint8)
            patches_dilated = np.zeros([buffer_size, patch_size, patch_size, patch_size], np.uint8)
            patch_num = cutils.get_patches_edge_dilated(output_shape, output_shape_edge, output_shape_dilated, patches, patches_edge, patches_dilated,
                                                        patch_size)
            ...
            for patch_id in range(patch_num):
                for compare_id in range(styleset_len_original):
                    ...
                    precision = torch.sum(torch.bitwise_and(patch_edge_tensor, patches_dilated_tensor), dim=(1, 2, 3), dtype=torch.int).float() / torch.sum(
                        patch_edge_tensor, dim=(1, 2, 3), dtype=torch.int).float()
                    recall = torch.sum(torch.bitwise_and(patch_dilated_tensor, patches_edge_tensor), dim=(1, 2, 3),
                                       dtype=torch.int).float() / patches_edge_sum_tensor
