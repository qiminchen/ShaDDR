...
def get_vox_from_binvox_512(objname):
...
def write_ply_point_color(name, vertices, colors):
    ...
    for ii in range(len(vertices)):
        fout.write(str(vertices[ii, 0]) + " " + str(vertices[ii, 1]) + " " + str(vertices[ii, 2]) + " " +
                   str(colors[ii,0]) + " " + str(colors[ii,1]) + " " + str(colors[ii,2]) + "\n")
    fout.close()
...
def write_obj_triangle_color(name, vertices, colors, triangles):
    fout = open(name, 'w')
    for ii in range(len(vertices)):
        fout.write("v "+str(vertices[ii,0])+" "+str(vertices[ii,1])+" "+str(vertices[ii,2])+" "+
                   str(int(colors[ii,0]))+" "+str(int(colors[ii,1]))+" "+str(int(colors[ii,2]))+"\n")
    for ii in range(len(triangles)):
        fout.write("f "+str(int(triangles[ii,0]+1))+" "+str(int(triangles[ii,1]+1))+" "+str(int(triangles[ii,2]+1))+"\n")
    fout.close()
...
class voxel_renderer:
    ...
    def render_img(self, voxel_in, threshold, view=0, get_depth=False, ray_x=0, ray_y=0, ray_z=1, steep_threshold=16):
        ...
        voxel[self.render_boundary_padding_size:-self.render_boundary_padding_size,
              ...
              self.render_boundary_padding_size:-self.render_boundary_padding_size] = (voxel_in > threshold).astype(np.uint8)[::-1,::-1]
    ...
    def render_img_with_camera_pose(self, voxel_in, threshold, cam_alpha=0.785, cam_beta=0.785, get_depth=False, processed=False,
                                    ray_x=0, ray_y=0, ray_z=1, steep_threshold=16):
        imgsize = voxel_in.shape[0]
...
def make_coordinates(H, W, D, normalize=True):
    xs = torch.arange(H).float()
    ys = torch.arange(W).float()
    zs = torch.arange(D).float()
    if normalize:
        xs = xs / (H - 1) * 2 - 1  # (-1, 1)
        ys = ys / (W - 1) * 2 - 1  # (-1, 1)
        zs = zs / (D - 1) * 2 - 1  # (-1, 1)

    coords = torch.stack(torch.meshgrid(xs, ys, zs), dim=-1).numpy()
